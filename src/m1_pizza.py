...
def run_test_generate_points_on_circle():
    ...
    expected = [rg.Point(155.0, 50.0),  # All numbers are approximate.
                ...
                rg.Point(140.0, 41.3),
                rg.Point(150.0, 41.3)]
    circle = rg.Circle(rg.Point(145, 50), 10)
    answer = generate_points_on_circle(circle, 6)
...
def draw_points_on_circle(window, circle, number_of_points, color):
    ...
    number_of_points = generate_points_on_circle(circle, number_of_points)
    for item in number_of_points:
        circle = rg.Circle(item, 10)
        circle.fill_color = color
        circle.attach_to(window)
        item.attach_to(window)
    window.render()
...
def run_test_pizza():
    ...
    # Test 4:
    title = ('PIZZA test 4: 8 slices, thin (thickness=5) black lines.' )
    window = rg.RoseWindow(400, 400, title)
...
def run_test_polygon():
    ...
    # -------------------------------------------------------------------------
    title = ('POLYGON test 3: 4 segments with thick black lines.')
    window = rg.RoseWindow(550, 400, title)

    circle = rg.Circle(rg.Point(120, 120), 90)
    circle.outline_thickness = 5
...
def run_test_fancy_polygon():
    ...
    title = ('FANCY POLYGON test 4:  8 lime black lines on red circle,'
             + ' hops = 3.')
    window = rg.RoseWindow(480, 350, title)

    circle = rg.Circle(rg.Point(240, 165), 150)
    circle.fill_color = 'red'
    fancy_polygon(window, circle, 20, 7, 'black', 5)
    window.close_on_mouse_click()
...
def fancy_polygon(window, circle, number_of_lines, hops_to_next_point, color,
    ...
    for i in range(len(points)):
        ...
        if i+hops_to_next_point >= len(points):
            point2 = points[i + hops_to_next_point - len(points)]
        else:
            point2 = points[i + hops_to_next_point]
        line = rg.Line(point1, point2)
        line.color = color
        line.thickness = thickness
        line.attach_to(window)
